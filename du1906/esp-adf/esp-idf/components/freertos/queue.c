...
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	...
	configASSERT( pxQueue );
	if (pxQueue->check_flag != 0xA6A6) {
		ets_printf("FREERTOS QUEUE: check_flag = %d\n", pxQueue->check_flag);
		assert(0);
	}
	...
	#if ( ( INCLUDE_xTaskGetSchedulerState == 1 ) || ( configUSE_TIMERS == 1 ) )
	{
       ...
       if (pxQueue->u.xSemaphore.xMutexHolder != xTaskGetCurrentTaskHandle()) {
               est_printf("Mutex holder: %p, Holder Task name: %s, Current Task Name: %s\n", xQueue, 
                                       pcTaskGetTaskName(pxQueue->u.xSemaphore.xMutexHolder), pcTaskGetTaskName(xTaskGetCurrentTaskHandle()) );
       }
