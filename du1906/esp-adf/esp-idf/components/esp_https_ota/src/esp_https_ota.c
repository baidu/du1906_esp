    ...
    if (process_again(status_code)) {
        while (1) {
            ...
            if (data_read < 0) {
                ...
                ESP_LOGE(TAG, "===============???????read > 0,    data_read = %d", data_read);
             }
        }
    }
    return ESP_OK;
...
static void _http_cleanup(esp_https_ota_t *handle)
{
...
static esp_err_t _http_reconnect(esp_https_ota_handle_t https_ota_handle)
{
    esp_err_t err = ESP_OK;
    if (NULL == https_ota_handle) {
        ESP_LOGE(TAG, "Reconnect parameters invalid");
        return ESP_ERR_INVALID_ARG;
    }
    esp_https_ota_t *handle = (esp_https_ota_t *)https_ota_handle;
    // close http client
    _http_cleanup(handle);

    //restart http connection
    handle->http_client = esp_http_client_init(&handle->http_cfg);
    if (handle->http_client == NULL) {
        ESP_LOGE(TAG, "Failed to reestablish connection");
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "Reconnect to ota offset: %d", handle->http_ota_offset);
    char rang_header[32];
    snprintf(rang_header, 32, "bytes=%d-", handle->http_ota_offset);
    esp_http_client_set_header(handle->http_client, "Range", rang_header);

    err = _http_connect(handle->http_client);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to reestablish HTTP connection");
        _http_cleanup(handle);
        return ESP_FAIL;
    }
    return ESP_OK;
...
esp_err_t esp_https_ota_begin(esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle)
{
    ...
    memcpy(&https_ota_handle->http_cfg, ota_config->http_config, sizeof(esp_http_client_config_t));
...
static esp_err_t _http_reconnect_loop(esp_https_ota_handle_t https_ota_handle)
{
    ota_exponent_backoff_init(32);
    while (true) {
        int delay = ota_exponent_backoff_get_next_delay();
        ESP_LOGI(TAG, "wait for reconnect interval time %ds", delay);
        if (ESP_FAIL == ota_timeout_checkpoint(delay)) {
            return ESP_FAIL;
        }
        vTaskDelay((delay * 1000) / portTICK_PERIOD_MS);
        ESP_LOGI(TAG, "start to reconnect ...");
        if (_http_reconnect(https_ota_handle) == ESP_OK) {
            ESP_LOGI(TAG, "http ota reconnect success");
            return ESP_OK;
        }
    }
    return ESP_FAIL;
...
esp_err_t esp_https_ota_begin_with_pos(esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle, size_t offset)
{
    ...
    /* Initiate HTTP Connection */
    https_ota_handle->http_ota_offset += offset;
...
esp_err_t esp_https_ota_get_img_desc(esp_https_ota_handle_t https_ota_handle, esp_app_desc_t *new_app_info)
{
    ...
    while (data_read_size > 0 && !esp_https_ota_is_complete_data_received(https_ota_handle)) {
         ...
         */
        int client_errno = esp_http_client_get_errno(handle->http_client);
        if (client_errno == ENOTCONN || client_errno == ECONNRESET || client_errno == ECONNABORTED) {
        ...
        if (data_read > 0) {
            data_read_size -= data_read;
            bytes_read += data_read;
            handle->http_ota_offset += data_read;
        } else {
            ESP_LOGE(TAG, "Fail to get img desc");
            return ESP_FAIL;
        }
    }
    if (data_read_size > 0) {
    ...
    switch (handle->state) {
        ...
        case ESP_HTTPS_OTA_IN_PROGRESS:
            ...
            if (data_read <= 0) {
                 ...
                 */
                int client_errno = esp_http_client_get_errno(handle->http_client);
                if (client_errno == ESP_FAIL) {
                    ESP_LOGE(TAG, "Fail to get errno");
                    return ESP_FAIL;
                }
                ...
                if ((client_errno == 0 || client_errno == ENOTCONN || client_errno == ECONNRESET || client_errno == ECONNABORTED || client_errno == EAGAIN || client_errno == ETIMEDOUT) && !is_recv_complete) {
                    ...
                    if (_http_reconnect_loop(https_ota_handle) == ESP_FAIL) {
                        ...
                        ESP_LOGI(TAG, "http ota reconnect success");
                        return ESP_ERR_HTTPS_OTA_RECONNECT;
                    }
