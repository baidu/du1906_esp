...
#if CONFIG_SPIRAM_USE_MALLOC
    ESP_LOGD(TAG, "Allocate task stack on ext memory");
    client->stack = heap_caps_malloc(client->config->task_stack, MALLOC_CAP_SPIRAM|MALLOC_CAP_8BIT);
    ESP_MEM_CHECK(TAG, client->stack, goto error);
    client->task = heap_caps_calloc(1, sizeof(StaticTask_t), MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT);
    ESP_MEM_CHECK(TAG, client->task, goto error);
    client->task_handle = xTaskCreateStaticPinnedToCore(esp_mqtt_task, "mqtt_task", client->config->task_stack, client, client->config->task_prio, client->stack, client->task, MQTT_TASK_CORE);
    ESP_MEM_CHECK(TAG, client->task_handle, goto error);
#else
...
{
    if (client->event.event_id == MQTT_EVENT_DISCONNECTED) {
        ESP_LOGE(TAG, "Invalied state for Publish, quit");
        return -1;
    }
