...
void esp_bt_check_need_restart()
{
    if(connection_is_alive() && (check_bt_is_alive()==false))
    {
        ...
        if (xPortGetCoreID()!=CONFIG_BTDM_CONTROLLER_PINNED_TO_CORE){
            ...
            while(times_out-- > 0 && g_int_enable_flag == 0 )
            {
                vTaskDelay(100/portTICK_PERIOD_MS);
            }
            RMT_DBG_LOG_ERROR("BT not alive,INT Core %d EN 0x%x",CONFIG_BTDM_CONTROLLER_PINNED_TO_CORE,g_int_enable_flag);
            intenable = g_int_enable_flag;
        }
