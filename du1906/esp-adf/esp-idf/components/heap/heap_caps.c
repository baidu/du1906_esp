...
{
    multi_heap_info_t info;
    ESP_LOGI(TAG, "Heap summary for capabilities 0x%08X:", caps);
    heap_t *heap;
    SLIST_FOREACH(heap, &registered_heaps, next) {
        if (heap_caps_match(heap, caps)) {
            multi_heap_get_info(heap->heap, &info);

            ESP_LOGI(TAG, "  At 0x%08x len %d free %d allocated %d min_free %d",
                   heap->start, heap->end - heap->start, info.total_free_bytes, info.total_allocated_bytes, info.minimum_free_bytes);
            ESP_LOGI(TAG, "    largest_free_block %d alloc_blocks %d free_blocks %d total_blocks %d",
